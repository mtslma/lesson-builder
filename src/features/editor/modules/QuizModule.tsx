        ...
        {block.items.map((it: any, i: number) => (
          ...
          >
            <input
              type="text"
              maxLength={2}
              className="absolute top-2 left-2 w-8 h-8 border-2 border-slate-300 text-center rounded-lg bg-white text-xs font-bold outline-none text-lime-600 focus:border-lime-500 shadow-xs"
              placeholder="#"
            />
            <img
              src={it.imageUrl}
              alt=""
              className="w-full h-28 object-cover rounded-lg bg-slate-200"
            />
          </div>
        ))}
      </div>
