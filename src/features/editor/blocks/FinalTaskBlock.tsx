...
export const FinalTaskForm: React.FC<any> = ({ block, onUpdate }) => (
  <div className="space-y-2">
    ...
    <textarea className="w-full p-2 border rounded text-sm h-16" value={(block.fields || []).join('\n')} onChange={(e) => onUpdate({ fields: e.target.value.split('\n').map((s: string) => s.trim()).filter(Boolean) })} placeholder="Fields (One per line)" />
  </div>
);
    ...
    <div className="space-y-3">
      {block.fields.map((f: string, i: number) => (
        <div key={i} className="flex items-center gap-3">
          ...
          <input type="text" className="flex-1 border-b-2 border-slate-200 bg-transparent outline-none focus:border-lime-500 py-1 text-sm font-sans" placeholder="..." />
        </div>
      ))}
    </div>
  </div>
